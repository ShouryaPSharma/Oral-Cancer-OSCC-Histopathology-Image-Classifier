...
Sample images are provided in the `demo_pics/` folder so you can quickly test the model on a few images without running the full pipeline.

`demo_model.py` exposes a single function, `apply_model(path)`, which loads the trained model and prints a prediction for the given image. To use it:
...
This will print the predicted class (Normal or OSCC) along with the model's confidence score. Make sure `saved_model.pth` (the trained weights) is present in the same directory before running this.
